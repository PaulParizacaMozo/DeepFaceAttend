...
const Register = () => {
  ...
  // --- Manejo de selección de fotos (Recuperación de Asistencias) ---
  // Recibe scheduleId para forzar selección única por grupo
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!profileImage) {
      setError(
        "Es necesario subir una foto frontal para el registro biométrico."
      );
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (recoverMode && role === "student" && selectedFaces.size > 0) {
        ...
        alert(
          `Registro exitoso con foto de perfil. Se han enviado ${recoveryData.length} asistencias para regularización.`
        );
      } else {
  ...
  const inputStyles =
    "block w-full appearance-none rounded-lg border border-gray-300 px-3 py-2.5 placeholder-gray-400 shadow-sm focus:border-primary focus:outline-none focus:ring-primary sm:text-sm";
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden">
      ...
      <div className="relative flex min-h-screen items-center justify-center p-4">
        ...
        <div
          className={`w-full transition-all duration-500 ease-in-out bg-white/95 rounded-2xl shadow-2xl ring-1 ring-black/5 overflow-hidden flex flex-col lg:flex-row ${
            step === 2 && recoverMode ? "max-w-6xl" : "max-w-md"
          }`}
        >
          {/* --- COLUMNA PRINCIPAL (FORMULARIO) --- */}
          <div
            className={`p-6 sm:p-10 transition-all duration-500 ${
              step === 2 && recoverMode
                ? "lg:w-1/3 border-r border-gray-200"
                : "w-full"
            }`}
          >
            ...
            >
              ...
              {step === 1 && (
                <div className="space-y-4 animate-fadeIn">
                  ...
                  {role === "student" && (
                    <div>
                      <label htmlFor="cui" className={labelStyles}>
                        CUI
                      </label>
                      <div className="mt-1">
                        <input
                  ...
                  <div className="pt-2">
                    <button
                      type="submit"
                      className="flex w-full justify-center rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-opacity-90 transition-colors"
                    >
                      Siguiente
                    </button>
                  </div>
                ...
                <div className="space-y-5 animate-fadeIn">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mb-2">
                      Necesaria para validar tu asistencia automáticamente.
                    </p>
                      ...
                      {previewUrl ? (
                        <div className="text-center relative">
                          <img
                            src={previewUrl}
                            alt="Preview"
                            className="h-32 w-32 object-cover rounded-full mx-auto shadow-md border-2 border-primary"
                          />
                          <button
                            type="button"
                            onClick={() => {
                      ...
                      <div className="flex items-center justify-between">
                        <button
                          ...
                          className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                            recoverMode ? "bg-primary" : "bg-gray-300"
                          }`}
                        >
                          ...
                          <span
                            ...
                            className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                              recoverMode ? "translate-x-5" : "translate-x-0"
                            }`}
                          />
                        </button>
                        <span
                          className={`ml-3 text-sm font-medium ${
                            recoverMode ? "text-primary" : "text-gray-500"
                          }`}
                        >
                          {recoverMode
                            ? "Recuperar Asistencia Activo"
                            : "Recuperar Asistencia"}
                        </span>
                      </div>
                      <p className="mt-2 text-xs text-gray-500 leading-tight">
                        Si asististe a clases sin estar registrado, activa esto
                        para identificarte en fotos pasadas.
                      </p>
                    </div>
                  )}
            ...
            <div className="lg:w-2/3 bg-gray-50 p-6 sm:p-10 flex flex-col h-full animate-fadeIn border-t lg:border-t-0 lg:border-l border-gray-200">
              <div className="mb-6">
                ...
                <p className="text-sm text-gray-600">
                  Selecciona tu foto en los horarios correspondientes. Solo
                  puedes seleccionar una foto por bloque de horario.
                </p>
              </div>
              ...
              >
                {MOCK_UNKNOWN_FACES.map((group, index) => (
                  <div
                    key={index}
                    className="mb-8 bg-white rounded-xl p-4 shadow-sm border border-gray-200"
                  >
                      ...
                      {group.faces.map((face) => {
                        ...
                        return (
                          <div
                            key={face.id}
                            onClick={() =>
                              toggleFaceSelection(face.id, group.schedule_id)
                            }
