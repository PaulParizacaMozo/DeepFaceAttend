...
CAMERA_INDEX = 0 # "http://192.168.1.46:8080/video" # Indice de la camara a usar
...
# --- 1. Hilo de la Camara (Siempre encendida) ---
def run_camera():
    global global_frame, global_camera
    print("[INFO] Hilo de la cámara iniciado.")

    # Crea una ventana que el usuario puede redimensionar manualmente
    ...
    while True:
        ...
        # Guarda el frame original (alta resolución) para el procesamiento
        with global_frame_lock:
            global_frame = frame.copy()

        # Muestra el frame en la ventana redimensionable
    ...
    # Destruye la ventana cuando el bucle termina
...
def process_frame_on_server(frame, schedule_id):
    try:
        ...
        print(f"[JOB] Enviando frame a {PROCESS_URL} (Schedule: {schedule_id})...")
        response = requests.post(PROCESS_URL, files=files, data=payload, timeout=10)
        if response.status_code == 200:
            data = response.json()
            return data.get('recognized_faces', [])
        else:
            print(f"[JOB] Error del servidor {PROCESS_URL}: {response.status_code}")
            return None
    except requests.exceptions.RequestException as e:
        print(f"[JOB] Error de conexión con el servidor {PROCESS_URL}: {e}")
        return None
...
if __name__ == '__main__':
    ...
    # app.run(host='0.0.0.0', port=6000, debug=True, use_reloader=False) # Descomenta para debug
    app.run(host='0.0.0.0', port=6000)
    
    print("[INFO] Servidor detenido, liberando cámara...")
